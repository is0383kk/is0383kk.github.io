---
title: "【Java】条件に沿ってランダムなパスワードを生成する"
...
## はじめに
今回は条件に沿ってランダムなパスワードを生成してみたいと思います。  
...
### パスワード生成で扱う文字の定義
まず、パスワード生成で扱う文字を「StringBuilderクラス」を使い定義します。
```java
StringBuilder passwordBase = new StringBuilder();
StringBuilder lowercaseBase = new StringBuilder("abcdefghijklmnopqrstuvwxyz");
StringBuilder uppercaseBase = new StringBuilder("ABCDEFGHIJKLMNOPQRSTUVWXYZ");
...
「passwordBase」は生成されるパスワードの文字を格納するための変数です。
...
ここでは、最初に定義した「文字種Base」に対してランダム関数で文字を抽出し、  
生成文字列となる「password」に直接特定文字種を挿入しています。
```java
// requiredが指定されている場合、必ず指定された文字種が含まれるようにする
if (lowercaseEnable == "reauired") {
    password.append(lowercaseBase.charAt(new Random().nextInt(lowercaseBase.length())));
...
また、今回Markdownでコード紹介を初めて行ったのですが、行番号付けられなかったりと色々不便だということが分かりました。  
